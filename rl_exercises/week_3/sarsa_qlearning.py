...
class TDAgent(AbstractAgent):
    ...
    def update_agent(self, batch) -> float:  # type: ignore
        ...
        if self.algorithm == "sarsa":
            ...
            next_action = self.policy(self.Q, next_state, evaluate=True)
    ...
    def SARSA(
        ...
        self.Q[state][action] += self.alpha * (
            reward
            + self.gamma * (self.Q[next_state][next_action] if not done else 0)
            - self.Q[state][action]
        )
    ...
    def Q_Learning(
        ...
        self.Q[state][action] += self.alpha * (
            reward
            + self.gamma * (self.Q[next_state][max_action] if not done else 0)
            - Q_s_a
        )
